...
async function parseSettings() {
  const settings = await getFileFromDB("system/settings.json");
  is12HourTime = JSON.parse(settings).timeFormat === "12h";
  imagePath = JSON.parse(settings).desktopBGPath;
}

async function loadDesktopBG() {
  let base64ImageData = await getFileFromDB(imagePath);
  ...
  const blob = new Blob([dataArray], { type: "image/jpg" });
...
async function populateMenu() {
  ...
  menu.innerHTML = "";
  const programListFile = await getFileFromDB("system/menuShortcuts.json");
  const programList = JSON.parse(programListFile);
  ...
  function appendToMenu(program, programData) {
    ...
    textContainer.innerHTML = programName.substring(
      0,
      programName.lastIndexOf(".")
    );
    menuItem.appendChild(textContainer);

    menuItem.classList = "listButton flexRow";
  ...
  for (let i = 0; i < programList.length; i++) {
    const programData = await getFileFromDB(programList[i]);
    if (programData !== null) {
...
function createWindow(currentWindowID, xPos, yPos, isAlert) {
  ...
  window.classList.add("window", "windowRidgeBorder");
...
function createHeader(currentWindowID, programName, buttonCount) {
  ...
  header.innerHTML = `<div class="menuHeaderTitle" style="${style}">${programName.substring(
    0,
    programName.lastIndexOf(".")
  )}<div>`;
  return header;
...
function createMenuBarButton(currentWindowID, programName) {
  ...
  menuBarButton.classList = "h-100 solidButton";
  ...
  document.getElementById("programArea").appendChild(menuBarButton);
}
...
function createAlert(text) {
  ...
  okButton.classList = " solidButton windowControlButton";
...
async function openProgram(program, withFile = null) {
  if (menuOpen) {
    toggleOpenmenu();
  }
  const programName = program.substring(program.lastIndexOf("/") + 1);
  const data = await getFileFromDB(program);
  ...
  window.appendChild(header);
  let windowCtrlContainer = document.createElement("div");
  windowCtrlContainer.classList.add("windowCtrlContainer");
  header.appendChild(windowCtrlContainer);
  ...
  windowCtrlContainer.appendChild(minimizeButton);

  let maximizeButton;
  if (!noResizeMatch) {
    maximizeButton = createMaximizeButton(currentWindowID, width, height);
    windowCtrlContainer.appendChild(maximizeButton);
  }

  const closeButton = createCloseButton(currentWindowID);
  windowCtrlContainer.appendChild(closeButton);
...
function registryResizingForProgram({
  ...
  programWindow.style.width = iframeInitSize.width + "px";
  programWindow.style.height = iframeInitSize.height + headerRectHeight + "px";
  ...
  const setCursor = (e) => {
    ...
    if (!e) {
      ...
      currentCursorStatus = checkCursorPosition(e, rect, resizableZoneWidth);
    }
  ...
  programWindow.addEventListener("mousedown", (e) => {
    ...
    const programWindowMinSize = {
      width: 210,
      height: headerRectHeight, // No border width, just header height
    };
...
function checkCursorPosition(e, rect, resizableZoneWidth) {
  ...
  const resizableRects = {
    topLeftCorner: {
      x,
      y,
      width: resizableZoneWidth,
      height: resizableZoneWidth,
    },
    topRightCorner: {
      x: x + width - resizableZoneWidth,
      y,
      width: resizableZoneWidth,
      height: resizableZoneWidth,
    },
    ...
    topBorder: {
      x: x + resizableZoneWidth,
      y,
      width: width - resizableZoneWidth * 2,
      height: resizableZoneWidth,
    },
    bottomBorder: {
      x: x + resizableZoneWidth,
    ...
    leftBorder: {
      x,
      y: y + resizableZoneWidth,
      width: resizableZoneWidth,
      height: height - resizableZoneWidth * 2,
    },
    rightBorder: {
      x: x + width - resizableZoneWidth,
      y: y + resizableZoneWidth,
...
function bringWindowToFront(winID, buttID) {
  ...
  windows.forEach(function (win) {
    ...
    if (win.id === winID) {
      win.style.zIndex = "5";
      win.classList.add("activeWindowShadow");
      win.classList.remove("inactiveWindowShadow");
    } else {
      win.style.zIndex = "4";
      win.classList.add("inactiveWindowShadow");
      win.classList.remove("activeWindowShadow");
...
function getClockEndText(is12HourTime, isPM) {
  return is12HourTime ? (isPM ? " PM" : " AM") : "";
}
...
function getDayStr(num) {
  return daysOfWeek[num] || "Invalid day";
}

function getMonthStr(num) {
  return monthsOfYear[num] || "Invalid month";
}
...
function updateClock() {
  ...
  const mins = checkTime(today.getMinutes());

  document.getElementById(
    "clock"
  ).innerHTML = `${dayOfWeek} ${month} ${day}<br/>${h}:${mins}${getClockEndText(
    is12HourTime,
    isPM
  )}`;
}
